...
class ConsultPipeline:
    ...
    async def pre_deploy(
        ...
        integration_results = await self._run_integration_tests(source)
    ...
    async def _run_integration_tests(
        ...
        """Run integration tests with Postgres service.
        ...
        if not migration_result[2]:  # Migration failed
            ...
            results.append(
                ("Integration", "Django health", False, "Skipped (migrations failed)")
            )
            results.append(
                ("Integration", "Migration check", False, "Skipped (migrations failed)")
            )
            results.append(
                ("Integration", "Worker health", False, "Skipped (migrations failed)")
            )
            return results

        # Run Django health check
        results.append(await self._check_django_health(django_container))

        # Run migration --check (verify no unapplied migrations)
        results.append(await self._check_migrations(django_container))

        # Run Worker health check (stateless, no DB needed)
        results.append(await self._check_worker_health(source))
    ...
    async def _check_django_health(
        ...
        """Check Django health by starting server and hitting admin login."""
        try:
            # Start Django server as a service
            django_service = (
                ...
                .with_exec(
                    ...
                    insecure_root_capabilities=True,  # Allow binding to port
                )
                .as_service()
            )

            # Create test container to check health
            test_container = (
                dag.container()
                .from_("curlimages/curl:latest")
                .with_service_binding("django", django_service)
            )

            # Wait for Django to be ready and check health
            # Try up to 30 times with 1 second delay
            health_check_script = (
                "for i in $(seq 1 30); do "
                "if curl -sf http://django:8000/admin/login/ >/dev/null 2>&1; then "
                "echo OK; exit 0; fi; sleep 1; done; echo TIMEOUT; exit 1"
            )
            output = await test_container.with_exec(
                ["sh", "-c", health_check_script]
            ).stdout()

            if "OK" in output:
                return ("Integration", "Django health", True, "")
            else:
                return ("Integration", "Django health", False, "Health check timed out")

        except Exception as e:
            error_msg = str(e)
            if "traceparent:" in error_msg:
                error_msg = error_msg.split("[traceparent:")[0].strip()
            return ("Integration", "Django health", False, error_msg)
    ...
    async def _check_worker_health(
        ...
        """Check Worker health endpoint (stateless, no DB needed)."""
        try:
            worker_src = source.directory("workers/intake").without_directory(
                "node_modules"
            )

            # Build worker container with wrangler
            worker_container = (
                dag.container()
                .from_("node:22-slim")
                .with_workdir("/app")
                ...
                .with_file("/app/tsconfig.json", worker_src.file("tsconfig.json"))
                .with_exec(["pnpm", "install"])
                # Set dummy env vars for local mode
                .with_env_variable(
                    "NEON_DATABASE_URL", "postgres://dummy:dummy@localhost/dummy"
                )
                .with_env_variable("INTAKE_API_KEY", "test-key")
            )

            # Create .dev.vars file and start worker in local mode
            dev_vars_script = (
                "echo 'NEON_DATABASE_URL=postgres://dummy:dummy@localhost/dummy'"
                " > .dev.vars && echo 'INTAKE_API_KEY=test-key' >> .dev.vars"
            )
            worker_service = (
                worker_container.with_exec(["sh", "-c", dev_vars_script])
                .with_exposed_port(8787)
                .with_exec(
                    [
                        "pnpm",
                        "wrangler",
            ...
            test_container = (
                dag.container()
                .from_("curlimages/curl:latest")
                .with_service_binding("worker", worker_service)
            )
            ...
            output = await test_container.with_exec(
                [
                    ...
                    "-c",
                    "for i in $(seq 1 30); do "
                    "if curl -sf http://worker:8787/health 2>&1 | grep -q 'ok'; then "
                    "echo 'OK'; exit 0; "
                    "fi; sleep 1; done; "
                    "echo 'TIMEOUT'; exit 1",
                ]
            ...
            if "OK" in output:
                return ("Integration", "Worker health", True, "")
            else:
                return ("Integration", "Worker health", False, "Health check timed out")

        except Exception as e:
            error_msg = str(e)
            if "traceparent:" in error_msg:
                error_msg = error_msg.split("[traceparent:")[0].strip()
            return ("Integration", "Worker health", False, error_msg)
